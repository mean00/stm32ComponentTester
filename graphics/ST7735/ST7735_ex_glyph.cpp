 ...
 */
int Adafruit_ST7735Ex::mySquare(int x, int y, int w, int xheight, uint16_t filler)
{
    if(w+x>=width())
    {
        w=width()-x;
...
int Adafruit_ST7735Ex::myDrawChar(int x, int y, unsigned char c,  int color, int bg,FontInfo &infos)
{
    c -= infos.font->first;
